...
import { readableStreamToString, DevError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new DevError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new DevError(e, e.response?.data)
                }
                if (innerError instanceof DevError) {
                    // Got "DevError" from the above try block
                    throw innerError;
                }
